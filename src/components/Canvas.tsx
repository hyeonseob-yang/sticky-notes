...
import { clampPosition, clampRectMaxSize, clampRectMinSize, normalizeRect } from '../utils/geometry'
...
export function Canvas({ onCreateNote, children }: CanvasProps) {
  ...
  // Normalizes the drag into a rect, floors it to the minimum note size, caps
  // it to at most the viewport size (so it's always possible to fully
  // contain), then keeps it fully on-screen (shifting the origin, if needed)
  // so a note dragged out near an edge isn't born already overflowing.
  const computeRect = (point: { x: number; y: number }): Rect => {
    ...
    const sized = clampRectMaxSize(clampRectMinSize(rect, MIN_WIDTH, MIN_HEIGHT), bounds)
    return clampPosition(sized, bounds)
  }
